...
\begin{itemize}
    ...
    \item \textbf{\texttt{pose.py}:} A \textit{Position} és \textit{Direction} kombinációja (pozíció és orientáció). Nélkülözhetetlen a jelzők és vonatok állapotának leírásához, ahol a térbeli elhelyezkedés mellett az irány is meghatározó.\cite{pose_stackexchange_definition}
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{impl/app_architecture/core_geometry_uml.png}
    \caption{A Core geometry UML osztálydiagramja}
    \label{fig:core-geometry-uml}
\end{figure}
...
\begin{itemize}
    \item \texttt{station\_repository.py}
    \item \texttt{signal\_repository.py}
    \item \texttt{train\_repository.py}
    \item \texttt{timetable\_repository.py}
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{impl/app_architecture/core_railway_uml.png}
    \caption{A Core railway UML osztálydiagramja}
    \label{fig:core-railway-uml}
\end{figure}
