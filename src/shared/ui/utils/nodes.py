...
def draw_node(screen: pygame.Surface, pos: Node | Position, camera: Camera, color: Color) -> None:
    """Draw a node on the given screen using the camera."""
    screen_x, screen_y = camera.world_to_screen(pos)
...
def draw_junction_node(screen: pygame.Surface, node: Node, camera: Camera, color: Color = Color.WHITE) -> None:
    """Draw a junction node on the given screen using the camera."""
