...
def draw_track(screen: pygame.Surface, world_edge: Edge, camera: Camera, edge_action: EdgeAction, length: int, speed: int = None) -> None:
    ...
    if edge_action is EdgeAction.BULLDOZE or edge_action is EdgeAction.INVALID_TRAIN_PLACEMENT:
        _draw_rail(screen, (a, b), camera, color=Color.RED, length=length, level=world_edge.level)
        ...
        _draw_rail(screen, (a, b), camera, color=Color.GREEN, length=length, level=world_edge.level)
    elif edge_action is EdgeAction.LOCKED:
        _draw_rail(screen, (a, b), camera, color=Color.LIME, length=length, level=world_edge.level)
    elif edge_action is EdgeAction.NO_SPEED:
        _draw_rail(screen, (a, b), camera, color=Color.WHITE, length=length, level=world_edge.level)
    elif edge_action is EdgeAction.SPEED:
        color = color_from_speed(speed)
        _draw_rail(screen, (a, b), camera, color=color, length=length, level=world_edge.level)
            
def _draw_rail(screen: pygame.Surface, screen_edge: tuple[Position, Position], camera: Camera, color: tuple[int, int, int], length: int, level: int) -> None:
    if level == 1:
    ...
    if length == Config.SHORT_SECTION_LENGTH:
        ...
        _draw_long_track(screen, screen_edge, color=color, width=max(1, 2*int(camera.scale)))
    else:
        raise NotImplementedError("Edge length drawing not implemented for length:", length)
    
def _draw_long_track(screen: pygame.Surface, screen_edge: Edge, color, width: int):
