    ...
    def render(self, world_pos: Position | None):
        if world_pos is None:
            if self._state.construction_anchor is not None:
                draw_node(self._screen, self._state.construction_anchor.position, self._camera, color=color_from_speed(self._state.track_speed))
            return
        ...
        if target.kind in (RailTargetType.NODE, RailTargetType.ANCHOR_SAME):
            draw_node(self._screen, target.snapped, self._camera, color=color)
            return
        
        if target.kind == RailTargetType.BLOCKED:
            draw_node(self._screen, target.snapped, self._camera, color=Color.RED)
            
            if self._state.construction_anchor is not None:
                draw_node(self._screen, self._state.construction_anchor.position, self._camera, color=color)
            return

        if target.kind == RailTargetType.NO_PATH:
            draw_node(self._screen, target.snapped, self._camera, color=Color.RED)
            if self._state.construction_anchor is not None:
                draw_node(self._screen, self._state.construction_anchor.position, self._camera, color=Color.RED)
            return
